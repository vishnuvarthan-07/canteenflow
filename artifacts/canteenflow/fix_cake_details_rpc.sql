-- Fix event_cake_details RLS policies
ALTER TABLE public.event_cake_details ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Enable read for all users" ON public.event_cake_details;
DROP POLICY IF EXISTS "Enable insert for all users" ON public.event_cake_details;
DROP POLICY IF EXISTS "Enable update for all users" ON public.event_cake_details;
DROP POLICY IF EXISTS "Enable delete for all users" ON public.event_cake_details;
DROP POLICY IF EXISTS "Allow anonymous all on event_cake_details" ON public.event_cake_details;

CREATE POLICY "Enable read for all users" ON public.event_cake_details FOR SELECT USING (true);
CREATE POLICY "Enable insert for all users" ON public.event_cake_details FOR INSERT WITH CHECK (true);
CREATE POLICY "Enable update for all users" ON public.event_cake_details FOR UPDATE USING (true);
CREATE POLICY "Enable delete for all users" ON public.event_cake_details FOR DELETE USING (true);


-- 1. UPDATE PLACE EVENT BOOKING RPC TO INCLUDE PRICE
...
DECLARE
  ...
  v_item JSONB;
BEGIN
  -- Validate Advance Booking: Event date must be in the future (current_date < event_date)
  -- Uses Asia/Kolkata timezone to prevent timezone bypass.
